...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    await logOrderActivity({
      ...
      performedBy: req.user?.userId || createdBy,
      metadata: {
        customerName: customer.name,
        productCount: enrichedProducts.length,
        source: source || null,
...
exports.updateOrderById = async (req, res) => {
  try {
    ...
    // ─────────────────────────────────────
    // CAPTURE OLD ORDER STATE
    // Used by dedicated order_activity logger
    // ─────────────────────────────────────
    const oldOrderSnapshot = {
      ...
      masterPipelineNo: order.masterPipelineNo,
      ...
      extraDiscountType: order.extraDiscountType,
      extraDiscountValue: order.extraDiscountValue,

      amountPaid: order.amountPaid,
      finalAmount: order.finalAmount,

      invoiceLink: order.invoiceLink,
      gatePassLink: order.gatePassLink,
      creditNoteLink: order.creditNoteLink,
      receivingDocumentLink: order.receivingDocumentLink,
    };
    ...
    if (updates.status) {
      ...
      // Invoice is mandatory before ANY dispatch status.
      if (
        ["DISPATCHED", "PARTIALLY_DISPATCHED"].includes(norm) &&
        !order.invoiceLink?.trim()
      ) {
        return sendErrorResponse(
          res,
          400,
          "Invoice must be uploaded before dispatching the order",
        );
      }

      // Gate-pass is also required when manually changing
      // the order status to a dispatched state.
      if (
        ["DISPATCHED", "PARTIALLY_DISPATCHED"].includes(norm) &&
        !order.gatePassLink?.trim()
      ) {
        return sendErrorResponse(
          res,
          400,
    ...
    // ─────────────────────────────────────
    // MASTER PIPELINE / PREVIOUS / QUOTATION
    // ─────────────────────────────────────
    if (updates.masterPipelineNo !== undefined) {
      if (!updates.masterPipelineNo) {
        ...
        const m = await Order.findOne({
          where: {
            orderNo: updates.masterPipelineNo,
          },
        });

        if (!m) {
          return sendErrorResponse(res, 404, "Master order not found");
        }
    ...
    if (updates.previousOrderNo !== undefined) {
      if (!updates.previousOrderNo) {
        ...
        const p = await Order.findOne({
          where: {
            orderNo: updates.previousOrderNo,
          },
        });

        if (!p) {
          return sendErrorResponse(res, 404, "Previous order not found");
        }
    ...
    if (updates.products !== undefined) {
      ...
      for (const p of updates.products) {
        ...
        const oldQty =
          order.products?.find((x) => (x.id || x.productId) === id)?.quantity ||
          0;
    ...
    const changedFields = Object.keys(updates).filter((field) => {
      const oldValue = oldOrderSnapshot[field];

      const newValue = newOrderSnapshot[field];
    ...
    await logActivity({
      ...
      metadata: {
        ...
        stockRestored:
          Array.isArray(oldOrderSnapshot.products) &&
          oldOrderSnapshot.products.length > 0,
      },
    ...
    if (changedFields.length > 0) {
      await logOrderActivity({
        ...
        description: `Order ${order.orderNo} updated: ${changedFields.join(
          ", ",
        )}`,

        oldValue: oldOrderSnapshot,

        newValue: newOrderSnapshot,
        ...
        metadata: {
          ...
          statusChanged: oldOrderSnapshot.status !== newOrderSnapshot.status,

          priorityChanged:
            oldOrderSnapshot.priority !== newOrderSnapshot.priority,

          productsChanged:
            JSON.stringify(oldOrderSnapshot.products) !==
            JSON.stringify(newOrderSnapshot.products),

          financialChanged:
            oldOrderSnapshot.shipping !== newOrderSnapshot.shipping ||
            oldOrderSnapshot.gst !== newOrderSnapshot.gst ||
            oldOrderSnapshot.extraDiscount !== newOrderSnapshot.extraDiscount ||
            oldOrderSnapshot.amountPaid !== newOrderSnapshot.amountPaid ||
            oldOrderSnapshot.finalAmount !== newOrderSnapshot.finalAmount,

          assignmentChanged:
            oldOrderSnapshot.assignedUserId !==
              newOrderSnapshot.assignedUserId ||
            oldOrderSnapshot.secondaryUserId !==
              newOrderSnapshot.secondaryUserId ||
            oldOrderSnapshot.assignedTeamId !== newOrderSnapshot.assignedTeamId,

          invoiceChanged:
            oldOrderSnapshot.invoiceLink !== newOrderSnapshot.invoiceLink,

          gatePassChanged:
            oldOrderSnapshot.gatePassLink !== newOrderSnapshot.gatePassLink,
    ...
    // ─────────────────────────────────────
    // UPDATE MONGODB ORDER ITEMS
    // Only if products changed
    // ─────────────────────────────────────
      ...
      dbProducts.forEach((p) => {
        ...
        if (p.images) {
          try {
            ...
            if (Array.isArray(imgs) && imgs.length > 0) {
              imageUrl = imgs[0]?.url || imgs[0] || null;
            }
          } catch (e) {}
        }
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    // CLOSED VALIDATION
    //
    // An order cannot be CLOSED until:
    //
    // 1. Invoice uploaded
    // 2. Gate-pass uploaded
    // 3. Receiving document uploaded
    // 4. All ordered quantities dispatched
    // =========================================================

    if (norm === "CLOSED") {
      const missingDocuments = [];

      if (!order.invoiceLink?.trim()) {
        missingDocuments.push("invoice");
      }

      if (!order.gatePassLink?.trim()) {
        missingDocuments.push("gate-pass");
      }

      if (!order.receivingDocumentLink?.trim()) {
        missingDocuments.push("receiving document");
      }
      ...
      if (missingDocuments.length > 0) {
        return sendErrorResponse(
          res,
          400,
          `Order cannot be closed. Upload the following first: ${missingDocuments.join(
            ", ",
          )}.`,
        );
      }
      ...
      for (const dispatch of order.dispatches || []) {
        for (const item of dispatch.items || []) {
          ...
          dispatchedMap[productId] =
            (dispatchedMap[productId] || 0) + Number(item.quantity || 0);
        }
      }
      ...
      for (const product of order.products || []) {
        ...
        const dispatchedQty = Number(dispatchedMap[productId] || 0);

        const remainingQty = Math.max(orderedQty - dispatchedQty, 0);
      ...
      if (undeliveredItems.length > 0) {
        const itemSummary = undeliveredItems
          .map((item) => `${item.name}: ${item.remaining} remaining`)
          .join(", ");

        return sendErrorResponse(
          res,
          400,
          `Order cannot be closed. All items must be fully dispatched first. ${itemSummary}`,
        );
      }
    }
    ...
    await logOrderActivity({
      ...
      description: `Order ${order.orderNo} status changed from ${oldStatus} → ${norm}`,
      ...
      metadata: {
        ...
        gatePassAvailable: !!order.gatePassLink,

        invoiceAvailable: !!order.invoiceLink,

        receivingDocumentAvailable: !!order.receivingDocumentLink,

        productCount: order.products?.length || 0,

        dispatchCount: order.dispatches?.length || 0,

        closeRequirements:
          norm === "CLOSED"
            ? {
                invoice: !!order.invoiceLink,
                gatePass: !!order.gatePassLink,
                receivingDocument: !!order.receivingDocumentLink,
