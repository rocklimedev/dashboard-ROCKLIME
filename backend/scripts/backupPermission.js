...
const Product = require("../models/permisson"); // adjust the path to your model
...
async function backupProducts() {
  try {
    ...
    const fileName = `permission_backup_${new Date()
      .toISOString()
      .replace(/[:.]/g, "-")}.json`;
    ...
    console.error("❌ Error during permissions backup:", err.message);
  } finally {
