...
function describeEvent(event) {
  if (event.type === 'assignment_change' && event.meta?.oldValue && event.meta?.newValue) {
...
export default function DeviceHistoryTimeline({ deviceId }) {
  ...
  const { data: events = [], isLoading } = useGetDeviceHistoryQuery({ deviceId, type, limit: 100 });
          ...
          items={events.map((event) => ({
            ...
            children: (
              <div>
                ...
                <div className="dm-mono" style={{ color: '#8B95A1', fontSize: 12 }}>
